...
<template>
  <nav sm:px3 flex="~ col gap2" shrink text-size-base leading-normal md:text-lg h-full mt-1 overflow-y-auto>
    ...
    <div class="spacer" shrink hidden sm:block />
    <NavSideItem text="Wordle" to="/wordle" icon="i-ri:gamepad-line" :command="command" />

    <div class="spacer" shrink hidden sm:block />
    <NavSideItem :text="$t('nav.settings')" to="/settings" icon="i-ri:settings-3-line" :command="command" />
    <NavSideItem :text="$t('settings.about.label')" to="/settings/about" icon="i-ri:information-line" :command="command" />
  </nav>
</template>
